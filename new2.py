...
class PiggyGameBot:
    ...
    def claim_share_bonus(self, account: Dict[str, str]) -> bool:
        """Menembak API untuk klaim bonus share +1 Play setelah game selesai"""
        try:
            headers = {
                'accept': '*/*',
                'accept-language': 'zh-CN,zh;q=0.9,en;q=0.8,zh-TW;q=0.7,ja;q=0.6,fr;q=0.5,ru;q=0.4,und;q=0.3',
            ...
            endpoint = "https://app.piggycell.io/api/trpc/piggyGame.addChargeCount?batch=1"
            data = {"0": {"json": None}}
            ...
            self.logger.info(f"账户 {account['name']} Mengirim request share bonus (+1 Play)...")
            response = session.post(
                endpoint,
                headers=headers,
                json=data,
            ...
            if response.status_code == 200:
                self.logger.info(f"账户 {account['name']} Berhasil menambah kuota bermain via fitur Share!")
                return True
            else:
                self.logger.warning(f"账户 {account['name']} Gagal menambah kuota share, Status: {response.status_code}")
                return False
                
        except Exception as e:
            self.logger.error(f"账户 {account['name']} Error saat klaim share bonus: {e}")
            return False
    ...
    def play_single_game(self, account: Dict[str, str], game_round: int) -> bool:
        ...
        try:
            ...
            if self.record_game_score(account, score):
                ...
                # --- FITUR OTOMATIS SHARE BONUS ---
                time.sleep(random.uniform(2.0, 4.0)) 
                self.claim_share_bonus(account)
                # ----------------------------------
